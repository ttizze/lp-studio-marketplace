...
3. **Create candidate DNA**
   - Produce 2-3 candidates when exploring.
   - Make the candidates meaningfully different in creative strategy, not only palette or section order. At least one candidate should be type-led or editorially expressive unless the category is regulated, enterprise-risk-heavy, or the user explicitly asks for restraint.
   ...
   - Produce 1 chosen DNA when the user asks for a decisive recommendation or implementation handoff.
   - Use `references/dna-framework.md` for archetypes, scoring, and the output schema.
...
4. **Score fit**
   ...
   - Pick the strongest candidate. If the highest score is not the recommendation, explain the tradeoff.

5. **Specify the selected DNA**
